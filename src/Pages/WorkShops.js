...
import dental_workshops from '../data/dental';
import nursing_workshops from '../data/nursing';
import bpt from '../data/bpt';
...
export default function BasicTabs() {
  ...
  return (
    ...
    <Box sx={{ width: '100%' }} id="workshops_container">
        ...
        <p id="subheading_">We offer 20+ workshops in the field of medical science at ARMSCON.</p>
      ...
      {
          nursing_workshops && nursing_workshops.map(index=><ActivityCard name={index.name}venue={index.venue}time={index.time}about={index.about}date={index.date}head_coordinator={index.hc}email_of_hc={index.email_of_hc} image={index.image}/>)
        }
      </CustomTabPanel>
      <CustomTabPanel value={value} index={3}>
      {
          bpt && bpt.map(index=><ActivityCard name={index.name}venue={index.venue}time={index.time}about={index.about}date={index.date}head_coordinator={index.hc}email_of_hc={index.email_of_hc} image={index.image}/>)
        }
      </CustomTabPanel>
      <CustomTabPanel value={value} index={4}>
      {
          dental_workshops && dental_workshops.map(index=><ActivityCard name={index.name}venue={index.venue}time={index.time}about={index.about}date={index.date}head_coordinator={index.hc}email_of_hc={index.email_of_hc} image={index.image}/>)
        }
      </CustomTabPanel>
