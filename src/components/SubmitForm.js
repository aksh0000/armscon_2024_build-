...
export default function SubmitForm() {
  ...
  console.log("THIS IS JSON DATA", json_Data.data_to_submit.data_to_send.dataOfPerson[2].profile_picture);

  function check_data(message) {
    let parsedMessage = JSON.parse(message);
    if (parsedMessage.message=="Signup successful") {
      //
  ...
  function POSTDATA(event) {
    ...
    fetch('https://www.armscon.org.in:5000/signup', {
      method: 'POST',
      body: formData,
    })
